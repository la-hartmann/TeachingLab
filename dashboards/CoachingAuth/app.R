...
fake_coaching_data <- readr::read_rds("data/fake_coaching_data.rds")
###

### Get Coaching data
coaching_data <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1fvy2NTZXs3zuIi9BE1XZP_0MZW7_7I_r8lrbjaqbtnU/edit#gid=1417501429") |>
  dplyr::bind_rows(fake_coaching_data)
###

### Get facilitator emails
coaching_emails <- gsub(
  " ",
  ".",
  paste0(tolower(sort(unique(coaching_data$Coach))), "@teachinglab.org")
) |>
...
ui <- tagList(
  ...
  navbarPage(
    ...
    title = titlePanel(h1("Coaching Feedback Sign-In", align = "center")),
    sidebarLayout(
      sidebarPanel(
        ### Size and position ###
        width = 3,
        ...
        shiny::selectizeInput(
          ...
          choices = coaching_data$`Content Area` |>
            unique() |>
            as.character() |>
            sort() |>
        ...
        splitLayout(
          ...
          shinyWidgets::airDatepickerInput(
            ...
            value = min(as.Date(coaching_data$Date), na.rm = T),
            minDate = min(as.Date(coaching_data$Date), na.rm = T),
            maxDate = max(as.Date(coaching_data$Date), na.rm = T) + 1,
      ...
      mainPanel(
        ...
        fluidRow(
          column(width = 4, shinyjs::hidden(gt::gt_output(outputId = "qualitative_table_2"))),
          column(width = 4, shinyjs::hidden(gt::gt_output(outputId = "qualitative_table_3"))),
          column(width = 4, shinyjs::hidden(gt::gt_output(outputId = "qualitative_table_1")))
        )
      )
    )
  )
...
server <- function(input, output) {
  ...
  n_size_data <- reactive({
    ...
    n_size <- nrow(
      coaching_data |>
        dplyr::filter(Coach == input$g.name)
      )
  ...
  filtered_data <- reactive({
    ...
    filtered_fac_data <- coaching_data |>
      dplyr::filter(Coach == input$g.name) |>
      ...
      dplyr::select(
        `They demonstrate deep knowledge of the content on which they coach`,
  ...
  qualitative_data_1 <- reactive({
    ...
    selected_feedback <- coaching_data |>
      dplyr::filter(Coach == input$g.name) |>
      dplyr::mutate(Date = as.Date(Date)) |>
      dplyr::filter(between(Date, input$date_min, input$date_max)) |>
      tlShiny::neg_cond_filter(
  ...
  qualitative_data_2 <- reactive({
    ...
    selected_feedback <- coaching_data |>
      ...
      dplyr::select(`What has gone well in your coaching sessions?`)
  ...
  qualitative_data_3 <- reactive({
    ...
    selected_feedback <- coaching_data |>
      ...
      dplyr::select(`What could have been better about your coaching sessions?`)
