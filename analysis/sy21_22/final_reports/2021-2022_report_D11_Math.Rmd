...
params:
  matched: "unmatched"
  partner: "District 11"
title: "District 11 Math Teaching Lab Report"
...
ipg_forms <- readr::read_rds(here::here("data/ipg_forms.rds")) |>
  filter(str_detect(`Name of Site (Parish, District, Network)`, "11")) |>
  dplyr::filter(Timestamp >= as.Date("2021-07-01") & str_detect(`IPG Rubric`, "Math")) |>
...
coaching_participant_feedback <- TeachingLab::get_coaching_feedback() |>
 filter(stringr::str_detect(Site, "asdfasdf")) # Get nothing on purpose
...
ongoing_participant_feedback <- TeachingLab::get_ongoing_coaching() |>
  filter(`Select the content area(s) for which you receive coaching support. - Math` == "Math" &
           str_detect(`Select your site (district, parish, network, or school).`, "11"))

### End of Session Read-in ###
session_survey <- TeachingLab::get_session_survey() |>
  filter(`Select the content area for today’s professional learning session.` == "Math" &
           str_detect(`Select your site (district, parish, network, or school).`, "11"))
...
diagnostic_survey <- TeachingLab::get_diagnostic_survey() |>
  filter(str_detect(your_site_district_parish_network_or_school_br_br, "11"))

### Follow Up Educator Read-in ###
followup_educator <- TeachingLab:::get_followup_educator() |>
  filter(str_detect(`Your site (district, parish, network, or school)`, "11"))
...
followup_educator <- followup_educator |>
  ...
  filter(`Which subject area do you teach/ lead? If you teach/support both, please select the one for which you received support from Teaching Lab this year.` == "Mathematics") ## Everything else is ELA so it must be this?

### Student Survey Read-in ###
d11_student_survey <- read_sheet("https://docs.google.com/spreadsheets/d/1XrIXFaP3z3LjX8h3VRh0PPiK6SxMUR7SqI1tYyufWds/edit#gid=228927115")
...
knowledge_assessments <- TeachingLab::get_knowledge_assessments() |>
  ...
  filter(n() > 10) |>
  ungroup() |>
  filter(str_detect(know_assess, "math|Math")) |>
...
if (sum(!is.na(diagnostic_survey$to_what_extent_do_you_agree_or_disagree_with_the_following_statements_i_am_color_blind_when_it_comes_to_my_teaching_i_don_t_think_of_my_students_in_terms_of_their_race_or_ethnicity)) >= 1) {
  ...
  mindsets_data_2 <- followup_educator |>
    select(all_of(teacher_mindsets_2))
  ...
  mindsets_n_1 <- paste0("n = ", max(mindsets_count_1$value))
  mindsets_n_2 <- paste0("n = ", max(mindsets_count_2$value))
...
After 14 middle schools adopted Illustrative Mathematics and approximately 70 teachers received TL professional learning, 84% of 46 teachers who were observed were effectively using IM learning goals and targets, 89% of teachers were effectively using IM Design Structure and 69% were effectively using IM instructional routines.



# Section 7: Student Outcomes

Teachers administered a student survey to at least one of their groups. Using Likert-type scales, students responded to items about their achievement identity, agency, growth mindsets, math enjoyment, math task persistence, metacognition, self-efficacy, student perception of teachers’ CRSE practices, and value and importance of mathematics. Responses were averaged across items. Higher scores correspond to more positive experiences.
...
growth_mindsets <- d11_student_survey |>
...
achievement_identity <- d11_student_survey |>
  select(20:21) |>
  summarise(across(everything(), ~ score_student_survey(.x, denom = 6))) |>
  pivot_longer(everything()) |>
  mutate(name = str_remove_all(name, "How much do you agree or disagree with the statements below\\? \\- "))

self_efficacy <- d11_student_survey |>
  select(22:24) |>
  summarise(across(everything(), ~ score_student_survey(.x, denom = 6))) |>
  pivot_longer(everything()) |>
  mutate(name = str_remove_all(name, "How much do you agree or disagree with the statements below\\? \\- "))

math_enjoyment <- d11_student_survey |>
  select(25:28) |>
  summarise(across(everything(), ~ score_student_survey(.x, denom = 4))) |>
  pivot_longer(everything()) |>
  mutate(name = str_remove_all(name, "How much do you agree or disagree with the statements below\\? \\- "))

meta_cognition <- d11_student_survey |>
  select(29:33) |>
...
value_mathematics <- d11_student_survey |>
  select(34:36) |>
...
math_task_persistence <- d11_student_survey |>
  select(37:39) |>
...
agency <- d11_student_survey |>
  select(40:41) |>
  summarise(across(c(1:2), ~ score_student_survey(.x, denom = 5))) |>
  pivot_longer(everything()) |>
  mutate(name = str_remove_all(name, "When reading the following statements, think about your current math class and decide how well the statements describe you\\. \\- "))

student_crse <- d11_student_survey |>
  select(42:43) |>
...
overall <- round(mean(c(growth_mindsets$value, achievement_identity$value, 
                      self_efficacy$value, math_enjoyment$value,
                      meta_cognition$value, value_mathematics$value,
                      math_task_persistence$value, agency$value,
                      student_crse$value)), 2)
...
tibble::tibble(
  ...
  `Average Score` = c(
    overall,
    round(mean(growth_mindsets$value), 2), 
    round(mean(achievement_identity$value), 2), 
    round(mean(self_efficacy$value), 2), 
    round(mean(math_enjoyment$value), 2),
    round(mean(meta_cognition$value), 2), 
    round(mean(value_mathematics$value), 2),
    round(mean(math_task_persistence$value), 2), 
    round(mean(agency$value), 2),
    round(mean(student_crse$value), 2))
  ) |>
  gt::gt() |>
