...
title: "Teaching Lab ELA (K-2) Report"
...
# d11_replace <- c("NYC District 11 - District-wide, NY" = "District 11",
#     "NYC District 11 - PS 89, NY" = "District 11", "NYC District 11 - PS 21, NY" = "District 11",
#     "NYC District 11 - IS 355, NY" = "District 11", "NYC District 11 - PS 96, NY" = "District 11",
#     "NYC District 11 - PS 87, NY" = "District 11", "Legacy Early College, SC" = "District 11", "NYC District 11 - PS 103, NY" = "District 11", "NYC District 11 - PS 189, NY" = "District 11",
#     "NYC District 11 - PS 498, NY" = "District 11")
...
if (params$partner != "All Partners") {
  ...
  matched_2021 <- read_rds(here::here("Data-Clean/Data-move/SY20-21/matched_2021.rds")) %>%
    filter((post_postelalevel == "Yes" & post_subject == "ELA/Literacy") | (pre_subject == "ELA/Literacy" & pre_preelalevel == "Yes"))
...
table_maker(data = mindsets_gt2, 
            ...
            bottom_row = 6) #%>%
  # gtsave("Downloads/quickimage.png")
```
...
# lableader_df <- splitstackshape::cSplit(lableader_df, "post_ll4a", ",") %>%
#   as_tibble() %>%
...
#   select(post_ll4a = sums, `n()/lableaders` = percent)
lableaders <- 7
lableader_df <- readr::read_rds(here::here("data-raw/lableader_df_ela2.rds"))
...
if (params$partner != "All Partners") {
  dashboard_data <- read_rds(here::here("Data-Clean/Data-move/Dashboard Data/dashboard_data.rds")) %>%
    dplyr::filter(str_detect(`District, Parish, Or Network`, "District 11")) %>%
...
```{r}
dashboard_gt <- dashboard_data %>%
  filter(`District, Parish, Or Network` == "Legacy Early College, SC") %>%
  mutate(across(where(is.character), ~ na_if(., "No Response"))) %>%
  summarise(across(c("How Likely Are You To Apply This Learning To Your Practice In The Next 4-6 Weeks?",
                     "I felt a sense of community with the other participants in this course even though we were meeting virtually.", 
                     "This course helped me navigate remote and/or hybrid learning during COVID-19.",
...
dashboard_gt %>%
  gt() %>%
  tab_header(
    gt::html(glue::glue("<strong>Teaching Lab Performance on Participant Feedback Questions for Legacy Early College"))
  ) %>%
  cols_label(
  `How Likely Are You To Apply This Learning To Your Practice In The Next 4-6 Weeks?` = 
    md("**How Likely Are You To Apply This Learning To Your Practice In The Next 4-6 Weeks?**"), 
...
```{r}
dashboard_gt <- dashboard_data %>%
  filter(`District, Parish, Or Network` == "Evangeline Parish, LA") %>%
  mutate(across(where(is.character), ~ na_if(., "No Response"))) %>%
  summarise(across(c("How Likely Are You To Apply This Learning To Your Practice In The Next 4-6 Weeks?",
                     "I felt a sense of community with the other participants in this course even though we were meeting virtually.", 
                     "This course helped me navigate remote and/or hybrid learning during COVID-19.",
