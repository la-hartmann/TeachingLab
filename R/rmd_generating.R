...
#' @param d27 site dropdown filter by default NULL and ignored
#' @param d11 site dropdown filter by default NULL and ignored
#' @param matched whether or not the responses should be matched, either "True" or "False"
...
partner_file_remove <- function(partner, content_area = NULL, d27 = NULL, d11 = NULL, matched = NULL, input, output_dir, ...) {
  print("Rendering new rmd...")

  output_file <- paste0("final_report_", stringr::str_replace_all(tolower(partner), c(
    " " = "_",
    "-" = "_",
    "," = "_"
  )))
...
#' @export
session_feedback_graph <- function(data, race_filter = "All Races", content_area_filter = "All") {
  session_survey <- data |>
...
#' @return prints a ggplot object
#' @export
course_feedback_graph <- function(data, race_filter = "All", content_area_filter = "All") {
  course_survey <- data |>
    dplyr::filter(last_session_or_not == "No - this was the final session for this PL course (e.g., last day of Bootcamp, close of Inquiry cycle) or last coaching session of the cycle." &
      course != "Coaching") |>
...
#' @return prints a ggplot object
#' @export
ongoing_coaching_feedback_graph <- function(data, race_filter = "All", content_area_filter = "All") {
...
#' @return prints a ggplot object
#' @export
end_coaching_feedback_graph <- function(data, race_filter = "All", content_area_filter = "All") {
  end_coaching <- data |>
    dplyr::filter(last_session_or_not == "No - this was the final session for this PL course (e.g., last day of Bootcamp, close of Inquiry cycle) or last coaching session of the cycle." &
      course == "Coaching") |>
    tidyr::drop_na(coach_end_feed_1)
